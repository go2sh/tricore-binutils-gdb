...
// TODO #define PCPVAL(i) pcp_insn.opexp[i].X_add_number
#define SP_REGNUM 10
...
// TODO static void pcp_init_word (int);
...
// TODO #define PCPTEXT_SECTION	6
// TODO #define PCPDATA_SECTION 7
...
// TODO   {"pcptext", make_section, PCPTEXT_SECTION},
// TODO   {"pcpdata", make_section, PCPDATA_SECTION},
// TODO   {"pcpinitword", pcp_init_word, 0},
  {"bit", create_bit, 1},
  {"lbit", create_bit, 0},
  {"bpos", bpcons, 1},
  {"bposb", bpcons, 1},
...
// TODO static segT pcptext_section;	/* PCP text section.  */
// TODO static segT pcpdata_section;	/* PCP data section.  */

#define AR_FLAGS		SEC_ALLOC | SEC_RELOC
#define ARLD_FLAGS		AR_FLAGS | SEC_LOAD | SEC_DATA
// TODO #define PCP_SEG			SEC_PCP
...
// TODO   {0, 1, ".pcptext",&pcptext_section,PCP_SEG | AR_FLAGS | SEC_LOAD | SEC_CODE},
// TODO   {0, 6, ".pcpdata",&pcpdata_section,PCP_SEG | ARLD_FLAGS}
};
...
static htab_t hash_ops = NULL;
static htab_t hash_sfr = NULL;
// TODO static htab_t hash_pcp = NULL;

/* This is used to quickly find compatible operand types.  */

static const char *pseudo_codes[256];
...
static int read_regno (char **);
static char read_regsuffix (char **);
static int get_expression (char *, char **, int);
static void tricore_ip (char *str);
static void insert_reg_or_const (int, int, int, char);
...
static int insert_abs18_14 (unsigned long *, int);
...
#if 0
TODO
static void pcp_ip (char *);
static int get_pcp_expression (char *, char *, int, int);
...
static htab_t hash_insn16 = NULL;
...
static htab_t hash_insn32 = NULL;
...
// TODO   pcp_opcode_t *ppop, *hppop;
  ...
  if (((hash_ops = str_htab_create ()) == NULL)
      || ((hash_sfr = str_htab_create ()) == NULL)
// TODO      || ((hash_pcp = str_htab_create ()) == NULL)
      || ((hash_insn16 = str_htab_create ()) == NULL)
      || ((hash_insn32 = str_htab_create ()) == NULL))
  ...
  for (opind = 0; opind < NUMOPCODES; ++opind)
    {
      ...
      str_hash_insert (hash_ops, pop->name, (char *) pop, 0);
...
TODO  
  ...
  for (opind = 0; opind < NUMPCPOPCODES; ++opind)
    {
      ...
      str_hash_insert (hash_pcp, ppop->name, (char *) ppop, 0);
  ...
  for (opind = 0; opind < NUMSFRS; ++opind, ++psfr)
    if (MATCHES_ISA (psfr->isa))
      {
        str_hash_insert (hash_sfr, psfr->name, (char *) psfr, 0);
      }
  ...
  for (opind = 0; opind < num_insn16; ++opind, ++pinsn16)
    {
      ...
      str_hash_insert (hash_insn16, pinsn16->name, (char *) pinsn16, 0);
  ...
  for (opind = 0; opind < num_insn32; ++opind, ++pinsn32)
    {
      ...
      str_hash_insert (hash_insn32, pinsn32->name, (char *) pinsn32, 0);
...
create_bit (int global)
{
  ...
  size_t len = strlen (bname) + 5;
  pname = alloca (len);
  snprintf (pname, len, "%s.pos", bname);
  *p = c;
  return;
  ...
  if (val == -1)
  ...
  else
    {
      ...
      bdata_sec = subseg_new (sname, 0);
      bfd_set_section_flags (bdata_sec,
...
int
tricore_elf_section_letter (int letter, const char **ptr_msg)
{
/* TODO
  if (letter == 'p')
...
flagword
tricore_elf_section_flags (flagword flags, int attr, int type ATTRIBUTE_UNUSED)
{
/* TODO	
...
*/
  ...
  if (attr & SHF_ABSOLUTE_DATA)
    flags |= SEC_TRICORE_ABSOLUTE_DATA;

  if (attr & SHF_CORE_NUMBER_MASK)
    flags |= SEC_TRICORE_CORE(SHF_CORE_NUMBER_GET(attr));
...
tricore_elf_section_change_hook ()
{
  ...
  old_align = bfd_section_alignment (now_seg);
  if (old_align > 0)
...
static int
get_expression (char *src, char **str, int opnr)
{
...
static void
insert_reg_or_const (int insert_reg, int regno, int  where, char mode)
{
  int new, old;
...
static int
insert_abs18 (unsigned long *code, int i)
{
...
static int
insert_abs18_14 (unsigned long *code, int i)
{
  unsigned long val = VAL (i);
...
int
md_estimate_size_before_relax (fragS *fragP, register segT segment)
{
  if (show_internals)
...
md_convert_frag (bfd *abfd ATTRIBUTE_UNUSED,
		segT segment ATTRIBUTE_UNUSED, fragS *fragP)
{
  char *buf = fragP->fr_opcode;
  unsigned char bytecode;
  int convert_insn = 0, pcrel = 1, regno;
  char optype, regtype;
  const char *opc16 = NULL, *opc_name = NULL;
...
static int
compare_relocs (const void *r1, const void *r2)
{
...
void
tricore_sort_relocs (asection *section, arelent **relocs, unsigned int n)
{
  rel_entry *srelocs;
  unsigned int i;
...
md_section_align (segT segment, valueT size)
{
  ...
  return ((size + (1 << align) - 1) & ((unsigned int) -1 << align));
}
...
int
tricore_fix_adjustable (fixS *fixP)
{
...
int
tricore_force_relocation (fixS *fixP)
{
  if ((fixP->fx_r_type == BFD_RELOC_VTABLE_INHERIT)
      || (fixP->fx_r_type == BFD_RELOC_VTABLE_ENTRY))
    return 1;
...
/* add the core number to all symbols which are defined in a core specific
   section
 */
void tricore_elf32_adjust_symtab(void)
{
  symbolS *symp;
  ...
  for (symp = symbol_rootP; symp; symp = symbol_next (symp))
    {
      ...
      /* if the symbol is defined pass the section coreid to the
         symbol
      */
      if (S_IS_DEFINED(symp) && !(bfdsym->flags & BSF_SECTION_SYM))
        {
          sec = S_GET_SEGMENT(symp);
          if (sec->flags & SEC_TRICORE_CORE_MASK)
            {
              unsigned int coreid = SEC_TRICORE_CORE_GET(sec->flags);
